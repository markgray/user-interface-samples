 ...
 * the Action Bar.
 *
 * This sample demonstrates ways to move a view in the z-axis using `setTranslationZ()`. This method
 * was introduced in API Level 21 ('Lollipop'). It uses two shapes, a circle and a square, and it
 * demonstrates two alternative ways to move a view in the z-axis. The first shape, the circle, has
 * a fixed elevation, which is defined in XML. The second view, the square, changes its elevatio
 * using `setTranslationZ` when a user touches it. The elevation reverts back once the touch is
 * removed.
 */
class MainActivity : SampleActivityBase() {
     ...
     * view to our layout file `R.layout.activity_main`. There are two of these:
     * layout/activity_main.xml used by displays narrower than 720dp, and displays 720dp or
     ...
     * `val transaction`, initialize our [ElevationBasicFragment] variable `val fragment` with a new
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            val fragment = ElevationBasicFragment()
