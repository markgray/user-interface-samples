...
class MainActivity : AppCompatActivity() {
     ...
     * root view holding a single [TextView] displaying the text: "This is an Activity of the
     * `AppWidget` sample. Long press the app icon and choose the widget picker to try the widgets
     * in the sample".
     *
     * We initialize our [ConstraintLayout] variable `rootView` to the view with ID
     * `R.id.root_view` then call [ViewCompat.setOnApplyWindowInsetsListener] to take
     ...
     * of `windowInsets` with [WindowInsetsCompat.Type.systemBars] as the argument, then
     * it updates the layout parameters of `v` to be a [ViewGroup.MarginLayoutParams]
     * with the left margin set to `insets.left`, the right margin set to `insets.right`,
     * the top margin set to `insets.top`, and the bottom margin set to `insets.bottom`.
